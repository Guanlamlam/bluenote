...
class LostFoundPostPage extends StatefulWidget {
  ...
  @override
  _LostFoundPostPageState createState() => _LostFoundPostPageState();
}
...
class _LostFoundPostPageState extends State<LostFoundPostPage> {
  ...
  Future<void> _submitPost() async {
    ...
    var contactNumber = _contactNumberController.text.trim();
    final location = _locationController.text.trim();

    // Prefix with +6 before storing
  ...
  Future<void> _saveToDraft() async {
    var contactNumber = _contactNumberController.text.trim();
    if (contactNumber.isNotEmpty && !contactNumber.startsWith('+6')) {
      contactNumber = '+6$contactNumber';
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Enter Contact Number (012-3456789)',
                  border: InputBorder.none,
                ),
                validator: (value) {
                  if (value == null || value.trim().isEmpty) {
                  ...
                  final pattern = RegExp(r'^01\d-\d{7}$');
              ...
              const SizedBox(height: 16),
              Row(children: [
                _buildTypeButton("Found", primaryColor),
                const SizedBox(width: 8),
                _buildTypeButton("Lost", primaryColor)
              ]),
              const Spacer(),
              Row(
                children: [
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Drafts', style: TextStyle(color: Colors.white)),
        backgroundColor: primaryColor,
